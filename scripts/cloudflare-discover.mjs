...
const matching = workers?.filter((w) => /powermatch/i.test(w.id));
console.log(
  JSON.stringify({
    workers: matching?.map((w) => ({ id: w.id, modified_on: w.modified_on })),
  }),
);
for (const worker of matching || []) {
  const secrets = await get(
    `/accounts/${account}/workers/scripts/${worker.id}/secrets`,
  );
  console.log(
    JSON.stringify({
      worker: worker.id,
      secret_names: secrets?.map((s) => s.name),
    }),
  );
}
const dbs = await get(`/accounts/${account}/d1/database`);
console.log(
  JSON.stringify({
    databases: dbs
      ?.filter((d) => /powermatch/i.test(d.name))
